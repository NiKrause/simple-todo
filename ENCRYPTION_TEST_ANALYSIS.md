...
const isDatabaseEncrypted = async (db) => {
	try {
		...
		const hasEntriesWithUndefinedValues = all.every(
			(entry) => entry.hash !== undefined && entry.value === undefined
		);
...
test('should handle wrong password gracefully', async ({ page: browserAPage }) => {
	...
	pageBrowserB.on('console', (msg) => {
		...
		if (
			text.includes('isDatabaseEncrypted') ||
	...
	if (!isPasswordModalVisible) {
		...
		const todoVisible = await pageBrowserB
			.locator(`text=${testTodoText}`)
			.isVisible()
			.catch(() => false);
...
const encryptionStatus = await pageBrowserB.evaluate(() => window.__debugEncryptionStatus());

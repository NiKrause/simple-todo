...
A tutorial chapter — the fifth. Each one adds a single idea to the one before it; this one takes the internet away. Two devices that have never met, no server, no relay, nothing arranging the introduction: somebody holds up a code, somebody else scans it, and a list crosses.
...
- **Your signature** — `@le-space/orbitdb-identity-provider-webauthn-did@0.5.0`  
...
- **Who may write** — `@orbitdb/core@^4.0.0`  
  A list is opened with write access for its owner alone. The other side replicates it and reads it, and cannot add a line.
- **The list itself** — `@orbitdb/core@^4.0.0`  
  A keyvalue store whose address is the hash of its own manifest. That is what makes an address impossible to guess, and therefore worth handing over.
- **Where it survives** — `helia@^7.1.0`, `blockstore-idb@^4.0.1`, `datastore-idb@^5.0.1`  
  Blocks and index live in this browser's IndexedDB. With no relay to fetch them back from, they have to be on the device or the list is gone at the next reload.
- **The handover** — `/simple-todo/qr01/1.0.0`  
  This app's own protocol, carrying versioned JSON envelopes dispatched on a kind field — so the list offer and everything after it share one stream instead of earning a protocol each.
- **Optional, and off** — `@le-space/aleph-bootstrap@0.9.4`  
  A directory lookup that finds a relay when the addresses baked into the build have rotated away. Nothing contacts it unless the box above is ticked.

...
host chain itself. This page follows the source.
...
- A handle says nothing about the value. Zama's documentation asks contracts to treat handles as
  opaque: equal handles imply equal values, but different handles do not imply different values, and
  the construction may change.
...
- `delegateForUserDecryption`: the delegator is `msg.sender`; delegator, delegate and
  ...
  contract; the only rule on the expiration date is that it lies in the future. Each (delegator,
  delegate, contract) can be delegated or revoked once per block. The Hardhat mock of this repository
  (host contracts 0.10.0) still demands one hour and has no wildcard; `@zama-fhe/sdk` also refuses
  less than one hour.
...
6. **Gateway.** The relayer sends `userDecryptionRequest` to the Gateway's `Decryption` contract in
   ...
   otherwise). It pins the current KMS context and emits `UserDecryptionRequest`. It does not check
   the ACL.
7. **KMS.** Each KMS node's connector sees the event and checks `ACL.isAllowed(handle, user)` and
   ...
   user decryption threshold is met: 9 of 13 in ProtocolConfig on Sepolia. The relayer collects the shares
...
decryption request and every KMS response as events. Testnet Gateway chain id 10901; the SDK's mainnet
preset uses 261131.
...
On chain, ProtocolConfig on Sepolia and on Ethereum mainnet registers 13 KMS signers with a user
decryption threshold of 9, a public decryption threshold of 7, a key generation threshold of 7 and an
MPC threshold of 4. Zama's Sepolia address page lists 13 KMS operators in its operator staking table: Zama, Dfns,
Figment, Fireblocks, InfStones, Unit410, LayerZero, Ledger, Omakase, Stake Capital, OpenZeppelin,
Etherscan and Conduit.
...
transaction and 5,000,000 of depth, the values Zama's documentation gives. HCULimit v0.3.0 also has a
per-block cap for non-whitelisted callers, set on both networks to 281,474,976,710,655 (2^48 - 1, the
largest `uint48`), so it does not limit anything today.
...
In v0.14.0 the Gateway no longer checks the user's signature itself; the KMS connector recovers it
with `ecrecover` and otherwise calls the account's ERC-1271 `isValidSignature` with a gas cap
(pull requests [#2624](https://github.com/zama-ai/fhevm/pull/2624),
...
threshold. Reading them depends on Zama's relayer, Gateway and key holders being online.

### Trust: technical

| Party | Trusted for | If it fails or misbehaves |
| --- | --- | --- |
| KMS operators | confidentiality of every value; correct decryption | the private key exists only as threshold shares held by them, so a group of colluding operators above the threshold could decrypt any ciphertext; Zama's documentation describes the protocol as robust while at most one third of the nodes are malicious. Too many nodes offline stops decryption. |
...
| Gateway | availability, ordering of requests | Zama describes it as trust-minimized and auditable; if it halts, inputs and decryptions stop |
...
How many KMS nodes would have to collude to decrypt is not stated in the pages cited here.
ProtocolConfig records an MPC threshold of 4 for 13 signers, and the reconstruction errors of
September 2026 show `n=13, deg=4`; whether the FHE key shares use that same degree was not verified,
so this page gives no collusion bound.
...
raised from `user_decryption_wasm.rs` and `threshold-algebra/src/poly.rs`. The ACL and the handles were correct on chain. The reporters read it
as nine shares received out of 13 with one inconsistent share and no error-correction budget left.
...
(Sepolia preset), `src/services/credential-service.ts` 47-48 (30-day defaults),
`src/services/decryption-service.ts` 40-42 (delegation propagation retry), `src/contracts/acl.ts` 70.
...
- `host-contracts/contracts/ACLEvents.sol`, `host-contracts/contracts/FHEEvents.sol`: event
  signatures.
- `gateway-contracts/contracts/Decryption.sol`: limits 127-137, public decryption request 340-360,
...
`FHEVMExecutor.sol`, `KMSVerifier.sol`, `HCULimit.sol`, `ProtocolConfig.sol` at tag v0.14.0.

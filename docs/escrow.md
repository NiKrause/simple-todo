...
| ----------- | ------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Creator     | Owns the todo, puts the money in, decides when it is paid out.      | The `msg.sender` of `lock`, in the app the passkey's account on Calibur, a smart contract by Uniswap Labs ([What Calibur is](passkey-account.md#calibur-simple)). Escrows are stored as `_escrows[creator][todoRef]`, so only the creator can `release` or `refund` (the lookup uses `msg.sender`).                                                                         |

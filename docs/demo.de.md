...
2. **Vollständiger Lauf.** `npm run smoke:sepolia`, gestartet von der Person, die den Schlüssel in
   `contracts/.env` verwaltet. Erwartet: jeder Schritt `ok` und am Ende `passed`. Der Ersteller hält
   nach dem Lauf vom 2026-09-16 kein cUSDTMock mehr, also wird wieder geprägt, genehmigt und verpackt:
   7 Transaktionen, am 2026-09-16 bei rund 1 gwei 0,00242 ETH. Die neuen Transaktions-Links aus der
   Ergebnistabelle notieren; sie können die Links in Szene 8 ersetzen.
...
**Zeigen.** Bob öffnet die Liste über ihre Adresse, sieht die Aufgabe als „Von Alice an Sie
delegiert“ und hakt sie ab. Sein Passkey fragt nach, der Header meldet „Änderung unterschrieben“. Bei
Alice erscheint die Aufgabe als erledigt.
...
verschlüsselten Anteil, ab 9 von 13 Anteilen setzt der Browser den Wert zusammen. Im Smoke-Test dauerte
...
bewegen. Die heute eingetragene Prüfstelle ist der Entwicklerschlüssel. In der Attrappe beantwortet die
...
eine ungedeckte Sperre sind nicht zu unterscheiden. Entschlüsselungsanfragen sind Transaktionen auf
Zamas Gateway-Chain und nennen dort Handle, Nutzeradresse und Transport-Schlüssel.
...
- Zamas KMS: 13 Betreiber halten den Entschlüsselungsschlüssel nur in Anteilen. Wie viele von ihnen
  zusammenwirken müssten, um ohne Berechtigung zu entschlüsseln, nennen die herangezogenen Quellen
  nicht; on-chain eingetragen sind Schwellenwerte von 9 (Nutzer-Entschlüsselung), 7 (öffentliche
  Entschlüsselung) und 4 (MPC).
...
- **Transport-Schlüssel zum Lesen:** im Browser der lesenden Person; das SDK speichert ihn laut Zamas
  Dokumentation standardmäßig unverschlüsselt in der IndexedDB.
...
- **Geplant:** ein Passkey-Konto (Calibur). Dort bleibt der ursprüngliche Einrichtungsschlüssel für
  immer Root-Key, die Nutzerverifikation des Passkeys wird on-chain nicht erzwungen, und fürs Lesen ist
  ein zusätzlicher Sitzungsschlüssel nötig, weil Zamas aktuelle Version nur ECDSA-Signaturen annimmt.
  Nicht eingebaut.

### Ist das reif für Mainnet?

Nein, es ist eine Testnetz-Vorführung. Zamas Protokoll läuft auf Ethereum Mainnet in derselben Version
v0.13 wie auf Sepolia. Offen sind: Die App ist nicht an die Chain angebunden, die Treuhand ist nicht
auditiert, die Prüfstelle ist ein Entwicklerschlüssel, die Passkey-Wallet fehlt, Version v0.14 ist
veröffentlicht, aber noch nicht bereitgestellt, und auf Mainnet bestätigt ein einziger
Coprozessor-Signierer jede verschlüsselte Eingabe (auf Sepolia 3 von 5). Nichts aus diesem Kapitel lief
auf Mainnet.
...
(<https://docs.zama.org/protocol/sdk/concepts/security-model>) und KMS

...
>
	...
	>
		<div class="flex items-center space-x-3">
			...
			<div class="rounded-lg bg-white p-2 shadow-lg border border-red-200">
				<svg width="20" height="22" viewBox="0 0 154 172" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			<div>
				<h3 class="text-lg font-bold text-[#E91315] dark:text-white" style="font-family: 'Epilogue', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">Storacha</h3>
				<p class="text-xs text-gray-600 dark:text-gray-300" style="font-family: 'DM Mono', monospace;">Keep it Spicy 🌶️</p>
			</div>
		</div>
	...
	{#if showStoracha}
		...
		{#if !$initializationStore.isInitialized}
			...
			>
				<div class="flex items-start space-x-3">
					<div class="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-amber-400 to-orange-500 shadow-lg">
						<Loader2 class="h-4 w-4 animate-spin text-white" />
					</div>
					<div class="flex-1 text-sm">
						<div class="font-semibold text-amber-900 dark:text-amber-100" style="font-family: 'Epilogue', sans-serif;">⚡ Database Initializing</div>
						<div class="mt-1 text-amber-800/90 dark:text-amber-200/90" style="font-family: 'DM Sans', sans-serif;">
							OrbitDB is still setting up your database. You can login to Storacha now, but backup & restore features will be available once initialization completes.
						</div>
						<div class="mt-2 flex items-center space-x-2">
							<div class="h-1.5 w-24 rounded-full bg-amber-200 dark:bg-amber-700">
								<div class="h-full w-3/4 rounded-full bg-gradient-to-r from-amber-400 to-orange-500 animate-pulse"></div>
							</div>
							<span class="text-xs text-amber-700 dark:text-amber-300" style="font-family: 'DM Mono', monospace;">Please wait...</span>
						</div>
					</div>
				</div>
			</div>
		...
		{#if error}
			...
			>
				<div class="flex items-start space-x-3">
					<div class="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-red-400 to-rose-500 shadow-lg">
						<AlertCircle class="h-4 w-4 text-white" />
					</div>
					<div class="flex-1">
						<div class="font-semibold text-red-900 dark:text-red-100" style="font-family: 'Epilogue', sans-serif;">❌ Error</div>
						<div class="mt-1 text-sm text-red-800/90 dark:text-red-200/90" style="font-family: 'DM Sans', sans-serif;">{error}</div>
					</div>
				</div>
			</div>
		{/if}
		...
		{#if success}
			...
			>
				<div class="flex items-start space-x-3">
					<div class="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 shadow-lg">
						<CheckCircle class="h-4 w-4 text-white" />
					</div>
					<div class="flex-1">
						<div class="font-semibold text-emerald-900 dark:text-emerald-100" style="font-family: 'Epilogue', sans-serif;">✅ Success</div>
						<div class="mt-1 text-sm text-emerald-800/90 dark:text-emerald-200/90" style="font-family: 'DM Sans', sans-serif;">{success}</div>
					</div>
				</div>
			</div>
		{/if}
		...
		{#if showProgress}
			...
			>
				<div class="flex items-start space-x-3">
					<div class="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 shadow-lg">
						<svg class="h-4 w-4 text-white animate-spin" fill="none" viewBox="0 0 24 24" stroke="currentColor">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
						</svg>
					</div>
					<div class="flex-1">
						<div class="mb-2 flex items-center justify-between">
							<span class="font-semibold text-indigo-900 dark:text-indigo-100" style="font-family: 'Epilogue', sans-serif;">
								🔄 {progressType === 'upload' ? 'Uploading' : 'Downloading'} Progress
							</span>
							<span class="text-sm font-medium text-indigo-700 dark:text-indigo-300" style="font-family: 'DM Mono', monospace;">
						...
						<div class="h-3 w-full rounded-full bg-indigo-100 shadow-inner dark:bg-indigo-800">
							<div
								class="h-3 rounded-full bg-gradient-to-r from-indigo-500 via-purple-500 to-blue-500 transition-all duration-500 ease-out shadow-sm"
								style="width: {progressPercentage}%"
							></div>
						</div>
						{#if progressCurrentBlock}
							<div class="mt-2 text-xs text-indigo-700/80 dark:text-indigo-300/80" style="font-family: 'DM Mono', monospace;">
						...
						{/if}
						{#if progressError}
							<div class="mt-2 rounded-md bg-red-100 px-2 py-1 text-xs text-red-700 dark:bg-red-900/30 dark:text-red-400" style="font-family: 'DM Sans', sans-serif;">
		...
		{#if !isLoggedIn}
			<!-- Login Section -->
			<div class="space-y-4">
				<div class="text-center text-sm text-gray-700 dark:text-gray-300" style="font-family: 'DM Sans', sans-serif;">
					🌶️ Connect to <span class="font-bold text-[#E91315]">Storacha</span> to backup your todos to hot decentralized storage!
				</div>

				<!-- Authentication Method Toggle -->
				<div class="mb-4 flex justify-center">
					<div class="rounded-lg border border-red-300 bg-gradient-to-r from-[#EFE3F3] to-[#FFE4AE] p-1 shadow-sm dark:from-gray-700 dark:to-gray-600">
						<button
							...
							class="rounded-md px-4 py-2 text-sm transition-colors {authMethod === 'credentials'
								...
								: 'text-gray-700 hover:bg-red-100 dark:text-gray-300 dark:hover:bg-gray-600'}" style="font-family: 'DM Sans', sans-serif;"
						>
							🔑 Credentials
						</button>
						<button
				...
				<div class="flex justify-center">
					<button
						...
						disabled={isLoading}
						class="flex items-center justify-center space-x-2 rounded-md bg-[#E91315] px-4 py-2 text-white transition-colors hover:bg-red-700 disabled:opacity-50 shadow-lg" style="font-family: 'Epilogue', sans-serif; font-weight: 500;"
					>
						<Key class="h-4 w-4" />
				...
				{#if showCredentialsForm}
					<div class="rounded-md border border-red-300 bg-gradient-to-br from-white to-[#EFE3F3] p-4 shadow-md dark:from-gray-700 dark:to-gray-600">
						{#if authMethod === 'credentials'}
							<!-- Credentials Form -->
							<h4 class="text-md mb-3 font-bold text-[#E91315] dark:text-white" style="font-family: 'Epilogue', sans-serif;">
								🔑 Storacha Key & Proof
							</h4>
							<div class="space-y-3">
								<input
									...
									class="w-full rounded-md border border-red-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-transparent focus:ring-2 focus:ring-red-500 focus:outline-none dark:border-gray-600 dark:bg-gray-800 dark:text-white" style="font-family: 'DM Mono', monospace;"
								/>
								<textarea
									bind:value={storachaProof}
									placeholder="Proof/Delegation (uCAIS...)"
									rows="3"
									class="w-full resize-none rounded-md border border-red-300 bg-white px-3 py-2 text-xs text-gray-900 focus:border-transparent focus:ring-2 focus:ring-red-500 focus:outline-none dark:border-gray-600 dark:bg-gray-800 dark:text-white" style="font-family: 'DM Mono', monospace;"
								></textarea>
								<div class="flex space-x-2">
									<button
										on:click={() => handleCredentialsLogin()}
										disabled={isLoading || !storachaKey.trim() || !storachaProof.trim()}
										class="flex-1 rounded-md bg-[#E91315] px-4 py-2 text-white transition-colors hover:bg-red-700 disabled:opacity-50 shadow-lg" style="font-family: 'Epilogue', sans-serif; font-weight: 600;"
									>
										{isLoading ? '🌶️ Logging in...' : '🔥 Login'}
									</button>
									<button
										on:click={clearForms}
										class="rounded-md bg-gray-300 px-4 py-2 text-gray-700 transition-colors hover:bg-gray-400 dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500" style="font-family: 'DM Sans', sans-serif;"
									>
										Cancel
									</button>
								</div>
			...
			<div class="space-y-4">
				...
				>
					<div class="flex items-center space-x-3">
						...
						<div>
							<div class="text-sm font-bold text-[#E91315] dark:text-white" style="font-family: 'Epilogue', sans-serif;">
								🌶️ Connected to Storacha
							</div>
							{#if currentSpace}
								<div class="text-xs text-[#0176CE] dark:text-gray-400" style="font-family: 'DM Mono', monospace;">
					...
					<button
						...
						class="flex items-center space-x-1 px-3 py-1 text-sm text-[#E91315] transition-colors hover:text-red-800 dark:text-red-400 dark:hover:text-red-300" style="font-family: 'DM Sans', sans-serif;"
					>
				...
				<div class="space-y-3">
					...
					<button
						...
						class="flex w-full items-center justify-center space-x-2 rounded-md bg-[#FFC83F] px-4 py-2 text-gray-900 transition-colors hover:bg-yellow-500 disabled:opacity-50 shadow-lg font-bold" style="font-family: 'Epilogue', sans-serif;"
					>
					...
					<button
						...
						class="flex w-full items-center justify-center space-x-2 rounded-md bg-[#0176CE] px-4 py-2 text-white transition-colors hover:bg-blue-700 disabled:opacity-50 shadow-lg font-bold" style="font-family: 'Epilogue', sans-serif;"
				...
				<div class="rounded-md border border-red-300 bg-gradient-to-br from-white to-[#FFE4AE] p-4 shadow-md dark:from-gray-700 dark:to-gray-600">
					<div class="mb-3 flex items-center justify-between">
						<h4 class="flex items-center space-x-2 font-bold text-[#E91315] dark:text-white" style="font-family: 'Epilogue', sans-serif;">
						...
						<div class="flex items-center space-x-1">
							<button
								...
								disabled={isLoading}
								class="rounded-md p-2 text-[#E91315] transition-all hover:bg-red-100 hover:scale-105 disabled:opacity-50 dark:text-red-400 dark:hover:bg-red-900/30" 
							...
							{#if spaceUsage && spaceUsage.totalFiles <= 50 && !spaceUsage.analyzed}
								<button
									...
									disabled={isLoading}
									class="rounded-md p-2 text-[#0176CE] transition-all hover:bg-blue-100 hover:scale-105 disabled:opacity-50 dark:text-blue-400 dark:hover:bg-blue-900/30"
						...
						>
							<div class="flex items-center justify-between text-sm">
								<div class="flex items-center space-x-2">
									...
									>
										<span class="text-xs font-bold text-white" style="font-family: 'DM Mono', monospace;"
											>{spaceUsage.totalFiles}</span
										>
									</div>
									<span class="font-medium text-gray-800 dark:text-gray-300" style="font-family: 'DM Sans', sans-serif;">
										🗺️ file{spaceUsage.totalFiles !== 1 ? 's' : ''} stored
									</span>
								</div>
								{#if spaceUsage.lastUploadDate}
									<div class="text-[#0176CE] dark:text-gray-400" style="font-family: 'DM Mono', monospace;">
							...
							{#if spaceUsage.totalFiles > 0}
								<div class="mt-2 grid grid-cols-3 gap-2 text-xs">
									{#if spaceUsage.backupFiles > 0}
										<div class="flex items-center space-x-1">
											...
											<span class="text-gray-700 dark:text-gray-300" style="font-family: 'DM Sans', sans-serif;"
								...
								<div class="mt-2 text-xs text-gray-600 dark:text-gray-400" style="font-family: 'DM Sans', sans-serif;">
									...
									{/if}
									<em class="text-gray-600 dark:text-gray-400">🌶️ Note: Each backup creates many data blocks</em>
								</div>
							{/if}
						</div>
					{:else if spaceUsage === null && isLoggedIn}
						...
						>
							<div class="flex items-center space-x-2">
								...
								<div class="text-sm font-medium text-[#E91315] dark:text-red-300" style="font-family: 'DM Sans', sans-serif;">

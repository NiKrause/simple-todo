...
class TutorialAutomator {
    ...
    async createSvelteProject(projectName) {
        ...
        try {
            ...
            const overwrite = await this.askUser(`⚠️  Directory "${projectName}" exists. Continue? (y/n): `);
    ...
    async copyFile(sourcePath, destPath) {
        ...
        try {
            ...
            console.log(`⚠️  Skipped ${sourcePath}: ${error.code === 'ENOENT' ? 'file not found' : error.message}`);
            return false;
        }
    }
    ...
    async copyFilesWithPrompt(stepFiles, stepName) {
        ...
        if (choice.toLowerCase() === 's' || choice.toLowerCase() === 'skip') {
            ...
            return 0;
        } else if (choice.toLowerCase() === 'a' || choice.toLowerCase() === 'all' || choice.toLowerCase() === '') {
            // Copy all files at once
            console.log('\n🚀 Copying all files...');
            for (const destPath of stepFiles) {
                const sourcePath = [...this.sourceFiles.entries()].find(([, dest]) => dest === destPath)?.[0];
                if (sourcePath) {
    ...
    async run() {
        ...
        try {
            ...
            const projectName = await this.askUser('📝 Enter project name (default: p2p-todo-demo): ') || 'p2p-todo-demo';
            ...
            if (startServer.toLowerCase() === 'y' || startServer.toLowerCase() === 'yes') {
                ...
                console.log('💡 Open http://localhost:5173 in multiple browser windows to see P2P magic!\n');
